...
Similarly to above, if we express $votes$ as a proportion of total votes, it is still justifiable to use a regression line, even if doesn't make any sense to receive 105% of the votes.  
...
two classes. (James et al., 2021, p. 132)
...
* The left-hand side of the logistic regression equation, which we call *logit* or *log odds* is 
    ...
    * in a logistic regression, a one-unit change in $X$, changes the *log odds* by $B_1$.
...
If the interpretation of the p-value doesn't change, the intepretation of the coefficients is differnet. Instead of a change in $Y$ for a one-unit change in $X$ is now a *log odds*, which we will need to transform to communicate 0-to-1 predictions!
...
> a student with an income and a balance of $2500 has an estimated 94.86% probability of defaulting on their credit card debt. 
...
Network data is data about relationships (or *edges*) among *vertices* (or *nodes*). Everything can be a node. And everything can be an edge. 
...
.center[<img src = '../img/newman-net-3.png' width = '80%'></img>]
